#' Parameter estimation based on the Clayton copula for serial dependence and the Frank copula for dependent censoring with the Weibull distribuions
#'
#' Perform two-stage estimation based on the Clayton copula C_theta for serial dependence and the Frank copula tilde(C)_alpha for dependent censoring with the marginal disributions Weib(r, nu_1) and Weib(lambda, nu_2). The jackknife method estimates the asymptotic covariance matrix. Parametric bootstrap is applied while doing Kolmogorov-Smirnov tests and Cramer-von Mises test. The guide for using this function shall be explained by Huang (2019).
#'
#' @usage ClaytonFrank.Weibull.MLE(subject, t.event, event, t.death, death, stageI, Weibull.plot,
...
#'
#' @examples
#' data = ClaytonFrank.Weibull.data(r = 1, nu_1 =0.5, theta = 2,
...
ClaytonFrank.Weibull.MLE = function(subject, t.event, event, t.death, death, stageI = "MLE", Weibull.plot = FALSE,
  ...
  result.estimate = c( exp(result.estimate.tilde[c(1,2,3)]), result.estimate.tilde[4], exp(result.estimate.tilde[c(5,6)]) )
  #####End Stage II
  ...
  if(jackknife == TRUE){#do not compute SE for subset
    ...
    for( i in c(1:N)){
      no = which(subject == unique(subject)[i])
      temp = Theta_k = NA
      temp = try(ClaytonFrank.Weibull.MLE(subject = subject[-no], t.event = t.event[-no], event = event[-no],
                                            t.death = t.death[-i], death = death[-i], stageI = stageI, jackknife = FALSE, digit = 10))
      if ("try-error"%in%class(temp)){
        jackknife.error = jackknife.error + 1
        next;
      }else if(temp$theta[1]>15|temp$theta[1]==0|temp$alpha[1]==0){
        jackknife.error = jackknife.error + 1
        next;
      }else{
        Theta_k = (log(c(temp$r[1], temp$nu_1[1], temp$theta[1], exp(temp$alpha[1]), temp$lambda[1], temp$nu_2[1])) - result.estimate.tilde)
  ...
  if( GOF == TRUE){
    ...
    for(b in c(1:rep.GOF)){
      set.seed(b)
      data_b = ClaytonFrank.Weibull.data(r = result.estimate[1], nu_1 = result.estimate[2], theta = result.estimate[3],
                                           lambda = result.estimate[5], nu_2 = result.estimate[6], alpha = result.estimate[4],
                                           N = N, b = max(t.death[death==0]), l = 200)
      result.bootstrap.temp = try(ClaytonFrank.Weibull.MLE(subject = data_b$Subject, t.event = data_b$T_ij, event = data_b$delta_ij,
