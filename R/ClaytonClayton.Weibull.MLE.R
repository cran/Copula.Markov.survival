#' Parameter estimation based on the Clayton copula for serial dependence and the Clayton copula for dependent censoring with the Weibull distribuions
#'
#' Perform two-stage estimation based on the Clayton copula C_theta for serial dependence and the Clayton copula tilde(C)_alpha for dependent censoring with the marginal disributions Weib(r, nu_1) and Weib(lambda, nu_2). The jackknife method estimates the asymptotic covariance matrix. Parametric bootstrap is applied while doing Kolmogorov-Smirnov tests and Cramer-von Mises test. The guide for using this function shall be explained by Huang (2019).
#'
#' @usage ClaytonClayton.Weibull.MLE(subject, t.event, event, t.death, death,
...
#'
#' @examples
#' data = ClaytonClayton.Weibull.data(r = 1, nu_1 =0.5, theta = 2,
...
ClaytonClayton.Weibull.MLE = function(subject, t.event, event, t.death, death,
  ...
  if(jackknife == TRUE){#do not compute SE for subset
    ...
    for( i in c(1:N)){
      ...
      if ("try-error"%in%class(temp)){
        ...
        Theta_k = (log(c(temp$r[1], temp$nu_1[1], temp$theta[1], temp$alpha[1], temp$lambda[1], temp$nu_2[1])) - result.estimate.tilde)
  ...
  if( GOF == TRUE){
    ...
    for(b in c(1:rep.GOF)){
      set.seed(b)
      data_b = ClaytonClayton.Weibull.data(r = result.estimate[1], nu_1 = result.estimate[2], theta = result.estimate[3],
