#' Parameter estimation based on the Frank copula for serial dependence and the Clayton copula for dependent censoring with the Weibull distribuions
#'
...
FrankClayton.Weibull.MLE = function(subject, t.event, event, t.death, death,
  ...
  result.estimate = c( exp(result.estimate.tilde[c(1,2)]), result.estimate.tilde[3], exp(result.estimate.tilde[c(4,5,6)]) )
  #####End Stage II
  ...
  if(jackknife == TRUE){#do not compute SE for subset
    ...
    for( i in c(1:N)){
      ...
      if ("try-error"%in%class(temp)){
        ...
        next;
      }else if(temp$alpha[1]>15|temp$theta[1]==0|temp$alpha[1]==0){
        jackknife.error = jackknife.error + 1
        next;
      }else{
        Theta_k = (log(c(temp$r[1], temp$nu_1[1], exp(temp$theta[1]), temp$alpha[1], temp$lambda[1], temp$nu_2[1])) - result.estimate.tilde)
